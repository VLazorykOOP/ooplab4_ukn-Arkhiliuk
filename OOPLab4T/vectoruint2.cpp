...
unsigned int VectorUInt::getSize() const { return size; }
...
VectorUInt VectorUInt::operator=(const VectorUInt &input) {
  if (size != input.size) {
    if (arr)
      delete[] arr;
    size = input.size;
...
VectorUInt VectorUInt::operator+(const unsigned int input) {
...
bool VectorUInt::operator==(const VectorUInt &input) {
  ui elems = size <= input.size ? size : input.size;
  for (ui i = 0; i < elems; i++) {
